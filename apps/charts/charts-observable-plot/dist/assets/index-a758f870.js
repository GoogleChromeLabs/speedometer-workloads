...
const K = 18, Xn = 0.96422, Yn = 1, Zn = 0.82521, t0$2 = 4 / 29, t1$2 = 6 / 29, t2 = 3 * t1$2 * t1$2, t3 = t1$2 * t1$2 * t1$2;
...
function formatYear$3(year2) {
  return year2 < 0 ? "-" + pad$3(-year2, 6) : year2 > 9999 ? "+" + pad$3(year2, 6) : pad$3(year2, 4);
}
function formatDate(date2) {
  ...
  return isNaN(date2) ? "Invalid Date" : formatYear$3(date2.getUTCFullYear()) + "-" + pad$3(date2.getUTCMonth() + 1, 2) + "-" + pad$3(date2.getUTCDate(), 2) + (milliseconds ? "T" + pad$3(hours, 2) + ":" + pad$3(minutes, 2) + ":" + pad$3(seconds, 2) + "." + pad$3(milliseconds, 3) + "Z" : seconds ? "T" + pad$3(hours, 2) + ":" + pad$3(minutes, 2) + ":" + pad$3(seconds, 2) + "Z" : minutes || hours ? "T" + pad$3(hours, 2) + ":" + pad$3(minutes, 2) + "Z" : "");
}
...
function clipCircle(radius2) {
  var cr = cos$1(radius2), delta = 2 * radians$1, smallRadius = cr > 0, notHemisphere = abs(cr) > epsilon;
...
const durationMinute$2 = durationSecond$1 * 60;
const durationHour$1 = durationMinute$2 * 60;
const durationDay$2 = durationHour$1 * 24;
...
const timeMinute = timeInterval((date2) => {
  ...
  return (end - start2) / durationMinute$2;
}, (date2) => {
...
const timeHour = timeInterval((date2) => {
  date2.setTime(date2 - date2.getMilliseconds() - date2.getSeconds() * durationSecond$1 - date2.getMinutes() * durationMinute$2);
}, (date2, step) => {
...
utcHour.range;
const timeDay$1 = timeInterval(
  (date2) => date2.setHours(0, 0, 0, 0),
  (date2, step) => date2.setDate(date2.getDate() + step),
  (start2, end) => (end - start2 - (end.getTimezoneOffset() - start2.getTimezoneOffset()) * durationMinute$2) / durationDay$2,
  (date2) => date2.getDate() - 1
);
timeDay$1.range;
const utcDay$2 = timeInterval((date2) => {
  date2.setUTCHours(0, 0, 0, 0);
}, (date2, step) => {
  date2.setUTCDate(date2.getUTCDate() + step);
}, (start2, end) => {
  return (end - start2) / durationDay$2;
}, (date2) => {
...
function timeWeekday(i) {
  return timeInterval((date2) => {
    ...
    return (end - start2 - (end.getTimezoneOffset() - start2.getTimezoneOffset()) * durationMinute$2) / durationWeek$2;
  });
}
...
function utcWeekday$1(i) {
  return timeInterval((date2) => {
    ...
    return (end - start2) / durationWeek$2;
  });
}
...
utcMonth.range;
const timeYear$1 = timeInterval((date2) => {
  date2.setMonth(0, 1);
  date2.setHours(0, 0, 0, 0);
}, (date2, step) => {
...
timeYear$1.range;
const utcYear$2 = timeInterval((date2) => {
...
function ticker(year2, month, week, day2, hour, minute) {
  ...
  function tickInterval(start2, stop, count) {
    ...
    if (i === tickIntervals2.length)
      return year2.every(tickStep(start2 / durationYear$1, stop / durationYear$1, count));
    if (i === 0)
...
const [utcTicks, utcTickInterval] = ticker(utcYear$2, utcMonth, utcSunday$1, unixDay, utcHour, utcMinute);
const [timeTicks, timeTickInterval] = ticker(timeYear$1, timeMonth, timeSunday, timeDay$1, timeHour, timeMinute);
var t0 = /* @__PURE__ */ new Date(), t1 = /* @__PURE__ */ new Date();
function newInterval(floori, offseti, count, field2) {
  function interval2(date2) {
    return floori(date2 = arguments.length === 0 ? /* @__PURE__ */ new Date() : /* @__PURE__ */ new Date(+date2)), date2;
  }
...
function formatLocale$1(locale2) {
  ...
  function newParse(specifier, newDate2) {
    return function(string2) {
      var d = newYear(1900), i = parseSpecifier(d, specifier, string2 += "", 0), week, day2;
      if (i != string2.length)
        return null;
      if ("Q" in d)
        return new Date(d.Q);
      ...
      if ("V" in d) {
        ...
        if (!("w" in d))
          d.w = 1;
        if ("Z" in d) {
          week = utcDate$1(newYear(d.y)), day2 = week.getUTCDay();
          ...
          week = newDate2(newYear(d.y)), day2 = week.getDay();
        ...
        day2 = "Z" in d ? utcDate$1(newYear(d.y)).getUTCDay() : newDate2(newYear(d.y)).getDay();
        d.m = 0;
...
function formatWeekNumberSunday$1(d, p) {
  return pad$2(sunday.count(timeYear(d), d), p, 2);
}
function formatWeekNumberISO$1(d, p) {
  var day2 = d.getDay();
  d = day2 >= 4 || day2 === 0 ? thursday(d) : thursday.ceil(d);
  return pad$2(thursday.count(timeYear(d), d) + (timeYear(d).getDay() === 4), p, 2);
}
function formatWeekdayNumberSunday$1(d) {
  return d.getDay();
}
function formatWeekNumberMonday$1(d, p) {
  return pad$2(monday.count(timeYear(d), d), p, 2);
}
...
function formatUTCWeekNumberSunday$1(d, p) {
  return pad$2(utcSunday.count(utcYear$1(d), d), p, 2);
}
function formatUTCWeekNumberISO$1(d, p) {
  var day2 = d.getUTCDay();
  d = day2 >= 4 || day2 === 0 ? utcThursday(d) : utcThursday.ceil(d);
  return pad$2(utcThursday.count(utcYear$1(d), d) + (utcYear$1(d).getUTCDay() === 4), p, 2);
}
function formatUTCWeekdayNumberSunday$1(d) {
  return d.getUTCDay();
}
function formatUTCWeekNumberMonday$1(d, p) {
  return pad$2(utcMonday.count(utcYear$1(d), d), p, 2);
}
...
Date.prototype.toISOString ? formatIsoNative : utcFormat$1(isoSpecifier);
function parseIsoNative(string2) {
  var date2 = new Date(string2);
  return isNaN(date2) ? null : date2;
}
+/* @__PURE__ */ new Date("2000-01-01T00:00:00.000Z") ? parseIsoNative : utcParse(isoSpecifier);
function date(t) {
  return new Date(t);
}
function number$1(t) {
...
function calendar(ticks2, tickInterval, year2, month, week, day2, hour, minute, second2, format2) {
  var scale = continuous(), invert = scale.invert, domain = scale.domain;
  var formatMillisecond = format2(".%L"), formatSecond = format2(":%S"), formatMinute = format2("%I:%M"), formatHour = format2("%I %p"), formatDay = format2("%a %d"), formatWeek = format2("%b %d"), formatMonth = format2("%B"), formatYear2 = format2("%Y");
  function tickFormat2(date2) {
    return (second2(date2) < date2 ? formatMillisecond : minute(date2) < date2 ? formatSecond : hour(date2) < date2 ? formatMinute : day2(date2) < date2 ? formatHour : month(date2) < date2 ? week(date2) < date2 ? formatDay : formatWeek : year2(date2) < date2 ? formatMonth : formatYear2)(date2);
  }
  scale.invert = function(y) {
  ...
  scale.copy = function() {
    return copy$1(scale, calendar(ticks2, tickInterval, year2, month, week, day2, hour, minute, second2, format2));
  };
  return scale;
}
function time() {
  return initRange.apply(calendar(timeTicks, timeTickInterval, timeYear$1, timeMonth, timeSunday, timeDay$1, timeHour, timeMinute, second$1, timeFormat$1).domain([new Date(2e3, 0, 1), new Date(2e3, 0, 2)]), arguments);
}
function utcTime() {
  return initRange.apply(calendar(utcTicks, utcTickInterval, utcYear$2, utcMonth, utcSunday$1, utcDay$2, utcHour, utcMinute, second$1, utcFormat$1).domain([Date.UTC(2e3, 0, 1), Date.UTC(2e3, 0, 2)]), arguments);
}
...
function formatWeekNumberSunday(d, p) {
  return pad$1(timeSunday.count(timeYear$1(d) - 1, d), p, 2);
}
function dISO(d) {
  var day2 = d.getDay();
...
function formatWeekNumberISO(d, p) {
  d = dISO(d);
  return pad$1(timeThursday.count(timeYear$1(d), d) + (timeYear$1(d).getDay() === 4), p, 2);
}
function formatWeekdayNumberSunday(d) {
  return d.getDay();
}
function formatWeekNumberMonday(d, p) {
  return pad$1(timeMonday.count(timeYear$1(d) - 1, d), p, 2);
}
...
function formatUTCWeekNumberISO(d, p) {
  d = UTCdISO(d);
  return pad$1(utcThursday$1.count(utcYear$2(d), d) + (utcYear$2(d).getUTCDay() === 4), p, 2);
}
function formatUTCWeekdayNumberSunday(d) {
  return d.getUTCDay();
}
function formatUTCWeekNumberMonday(d, p) {
  return pad$1(utcMonday$1.count(utcYear$2(d) - 1, d), p, 2);
}
...
const utcIntervals = /* @__PURE__ */ new Map([
  ...
  ["day", unixDay],
  ["monday", utcMonday$1],
  ["tuesday", utcTuesday$1],
  ["wednesday", utcWednesday$1],
  ["thursday", utcThursday$1],
  ["friday", utcFriday$1],
  ["saturday", utcSaturday$1],
  ["sunday", utcSunday$1],
  ["week", utcSunday$1],
  ["month", utcMonth],
  ["year", utcYear$2]
...
const timeFormatIntervals = [
  ["year", timeYear$1, "time"],
  ["month", timeMonth, "time"],
  ["day", timeDay$1, "time", 6 * durationMonth],
...
function gridKy(k2, anchor, data, {
  ...
  return axisMark(ruleY, k2, data, { ariaLabel: `${k2}-grid`, ariaHidden: true }, { y, x1: x12, x2, ...gridDefaults(options) });
}
function gridKx(k2, anchor, data, {
  ...
  return axisMark(ruleX, k2, data, { ariaLabel: `${k2}-grid`, ariaHidden: true }, { x, y1: y12, y2, ...gridDefaults(options) });
}
